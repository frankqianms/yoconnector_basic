 ...
 */
interface IYoconnectorBasicConnectorData {
    webhookUrl: string;
    user: string;
    appType: string;
    groupName: string;
...
/**
 * Implementation of the "YoconnectorBasicConnectorConnector" Office 365 Connector
 */
@ConnectorDeclaration(
    "/api/connector/connect",
    "/api/connector/ping"
)
@PreventIframe("/yoconnectorBasicConnector/config.html")
export class YoconnectorBasicConnector implements IConnector {
    ...
    public Ping(req: Request): Array<Promise <void>> {
        ...
        try {
            this.connectors.push("/connectors",
                (this.connectors.getData("/connectors") as IYoconnectorBasicConnectorData[])
                    .filter(c => {
                        return c.existing;
                    }));
        } catch (error) {
        ...
        return (this.connectors.getData("/connectors") as IYoconnectorBasicConnectorData[]).map((connector, index) => {
